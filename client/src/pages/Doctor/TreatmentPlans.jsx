...
import { treatmentPlans, exercises } from '../../data/exercises';
        ...
        {plans.map((plan) => (
          <div key={plan.id} className="bg-white rounded-lg shadow p-6 space-y-4">
            <div className="flex items-start justify-between">
              ...
              <span className="px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">{plan.status}</span>
            </div>
                  ...
                  return (
                    <div key={i} className="p-3 border border-gray-200 rounded-lg">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-800">{meta?.difficulty || 'N/A'}</span>
                      </div>
                      <p className="text-xs text-gray-600 mt-1">{ex.frequency} • {ex.sets} × {ex.reps} • {ex.duration}</p>
                      {ex.notes && <p className="text-xs text-gray-700 mt-1">Notes: {ex.notes}</p>}
                    </div>
                  );
                })}
              </div>
            ...
            {plan.progressNotes?.length > 0 && (
              <div>
                ...
                <div className="space-y-2">
                  {plan.progressNotes.map((n, i) => (
                    <div key={i} className="p-3 bg-gray-50 rounded">
                      <p className="text-xs text-gray-500 mb-1">{n.date}</p>
                      <p className="text-sm text-gray-900">{n.notes}</p>
                      {n.nextSteps && <p className="text-xs text-gray-700 mt-1">Next: {n.nextSteps}</p>}
                    </div>
                  ))}
                </div>
              </div>
